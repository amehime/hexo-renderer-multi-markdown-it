    ...
    md.renderer.rules.fence = (tokens, idx, options, env, self) => {
        ...
        if(code) {
            ...
            for (let i = 0, len = lines.length; i < len; i++) {
                ...
                if (command) {
                    if(command[lineno]) {
                        content += `<td data-num="${lineno}"></td><td data-command="${command[lineno]}">`;
                    } else {
                        content += `<td data-num="${lineno}" colspan="2">`;
                    }
                } else {
                    content += `<td data-num="${lineno}">`;
                }

                content += `</td><td><pre>${line}</pre></td></tr>`;                
            }
