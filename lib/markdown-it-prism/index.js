...
module.exports = function (md, options) {
    ...
    md.renderer.rules.fence = (tokens, idx, options, env, self) => {
        ...
        if (prismLang) {
            ...
            const code = Prism.highlight(text, prismLang, langToUse);
            ...
            for (let i = 0, len = lines.length; i < len; i++) {
                ...
                numbers += `<span></span>`; // ${Number(firstLine) + i}
                commands += `<span`;
                if (command && command[lineno]) {
                    commands += ` data-command="${command[lineno]}"></span>`;
                } else {
                    commands += `></span>`;
                }
            }

            let result = `<figure class="highlight${langToUse ? ` ${langToUse}` : ''}" data-language="${langShow}">`;
            ...
            if (command) {
                result += `<span class="command-line-prompt">${commands}</span>`;
            }
